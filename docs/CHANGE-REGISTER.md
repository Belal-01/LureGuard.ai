...
- ✅ **ARC-6** Topology
- ✅ **ARC-7** Every host's local events shared one rolling window
- ✅ **ARC-8** A single loud source could saturate the sensor — a DoS against the detector ·  ✓check
...
- ✅ **ING-8** Every Telegram alert blocked the whole event loop ·  ✓check
...
- ✅ **ML-12** Trained on a published dataset, evaluated across held-out testbeds ·  ✓check
- ✅ **ML-2** The informative features were computed and discarded ·  ✓check
...
| ML-11 | ↩️ Reversed | **Bounded web-noise suppression, withdrawn when the model it depended on was replaced.** The original fix let a high-confidence model score suppress level-10 web noise (31151, "multiple 400 errors from same source ip"), bounded three ways: confidence < 0.1, never above level 12, and always recorded. That was sound for the model it was built around — a *web-noise estimator* whose low score meant "this web traffic looks benign". The 2026-08-17 model is a **post-exploitation detector trained with scan and flood phases excluded**, so it scores ordinary web traffic near zero by construction (p=0.005 on a routine event). Under the old branch that reads as maximum confidence to suppress, and **every level-10 web detection was silently dropped** — ML-8 reached by a different road. Caught by ML-10's check, not by review. Suppression is withdrawn entirely; the model may now only *add* alerts. The real defect was not a mis-tuned threshold but that the alerting path was coupled to what one model meant by a low score, and nothing re-checked that meaning when the model changed. The replacement check asserts the coupling-free invariant: across the full score range 0.0–1.0 and levels 10–15, a rule-confirmed detection always reaches the operator. |
| ML-12 | ✅ Fixed | **Trained on a published dataset, evaluated across held-out testbeds.** AIT Alert Data Set (Zenodo 8263181, CC-BY-4.0 — Landauer, Skopik & Wurzenberger): 2.7 GB, ~2.6M real Wazuh alerts, 8 independent testbeds, 79 labelled attack phases across 10 attack types. Retargeted from web to **post-exploitation** (privilege_escalation, service_stop, webshell, reverse_shell) after measuring that web could not support a classifier: of 1,698,882 web alerts only **643 are benign** (0.04%) because in these testbeds nobody browses the web apps except the attacker — "web" and "attack" are the same fact twice. Scan/flood phases excluded from both classes (98% of the corpus; Wazuh rules already catch them in 1–2s). Trained on 583,093 rows (256 post-exploitation / 582,837 benign) from 6 testbeds; **santos and wheeler held out entirely**. Labels derive only from published attack-window timestamps — the Wazuh verdict is not reachable from the labelling function, so ML-1's leak cannot return. Held-out result: **post-exploitation detection 2/8 → 6/8** (Wazuh floor 2/8, model 3/8, evidence rule `sudo/su/pam` 4/8, union 6/8) with **false alerts 3,984 → 150–249 of 299,902** benign events. The model is the only detector that finds wheeler/webshell, in 3s. Accuracy is deliberately not reported: at a 0.032% base rate an always-benign model scores 99.968%. Honest ceiling — 79 attack phases, 256 positives, 8 held-out runs; the model-vs-rule gap is a single run and is not a ranking. `service_stop` is undetectable from this corpus (4 alerts total). Reproducible via `ml/train_postexploit.py`, 9m07s. |
...
| ARC-7 | ✅ Fixed | **Every host's local events shared one rolling window.** `extract_event_features` keyed the window on `event.src_ip or "0.0.0.0"`, so every alert without a source IP — `sudo`, `su`, `pam`, FIM, the bulk of host-local activity — fell into a single shared bucket. That fused the whole fleet into one fictitious source: f1 became a fleet-wide event count and f3 a union of usernames from unrelated machines, so a busy web server could mask privilege escalation on a database server and no host's temporal baseline meant anything. Now keyed per agent when no source IP is present (`_window_key`). Measured consequence: post-exploitation becomes visible to the model at all — under the shared key it was not. This also unblocked ML-12, whose trainer had been *discarding* these alerts to avoid the mega-source, which threw away exactly the attacks worth detecting (only 28 of 158 privilege-escalation alerts carry a source IP). |
| ARC-8 | ✅ Fixed | **A single loud source could saturate the sensor — a DoS against the detector.** f1–f6 were recomputed by scanning the whole per-IP window, so per-event cost grew with the window and the window grew with the attacker's rate. Measured: **8,745 ev/s at 2 req/s but 505 ev/s at 60 req/s** (18k-event window); extrapolated, ~175 req/s from one IP saturates a core. The detector degraded fastest exactly when under attack, and it is not hypothetical — dirb in the fox testbed runs at 52 req/s and builds a 15,600-event window, which is why a training run stalled for 17 minutes. Window capped at 2,000 events per source (`MAX_WINDOW_EVENTS`): throughput is now flat ~3,000 ev/s from 2 to 200 req/s. f1 becomes `min(events in window, cap)` — no decision reads the magnitude. Separately, `statistics.pstdev` promoted every sample to `Fraction` and was **49% of feature-extraction time** on the live ingest path; replaced with plain float arithmetic, 10.6× faster on that call, max deviation 1.4e-14. Check asserts the *property* — throughput must not collapse between 2 and 120 req/s — so a reintroduced unbounded scan fails even if the constant survives. |
